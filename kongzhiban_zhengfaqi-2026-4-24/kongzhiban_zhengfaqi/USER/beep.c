...
void f_Beep(unsigned char const i_MusicNum)
{
	...
	if(i_MusicNum >= MAX_BEEP_NO)
		return;
	
	pMusicAddr = MusicSoundTab[i_MusicNum];
...
void f_OutBuz(void)
{
	if(nMusicPos)
	{
		if(nMusicDelayTm >= pMusicAddr[nMusicPos])
		{
			nMusicPos ++;
			nMusicDelayTm = 0;
			if(pMusicAddr[nMusicPos] == 0)
			{
				nMusicPos = 0;
				f_MCU_OutMusicPWM(0);
			}
			else
			{
				...
				{
					f_MCU_OutMusicPWM(pMusicAddr[nMusicPos]);
				}
				nMusicPos ++;
				nMusicDelayTm = 0;
			}
