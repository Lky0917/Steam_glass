...
void f_ChangeKeyInf(unsigned long *KeyInfDeal)
{
	...
	if(MchInf.KeyState&0x8000)
		tempkey |=0x01;
	if(MchInf.KeyState&0x4000)
		tempkey |=0x02;
	if(MchInf.KeyState&0x2000)
		tempkey |=0x04;	
	if(MchInf.KeyState&0x1000)
		tempkey |=0x08;
	
	if(MchInf.KeyState&0x0800)
		tempkey |=0x10;
	if(MchInf.KeyState&0x0400)
		tempkey |=0x20;
	if(MchInf.KeyState&0x0200)
		tempkey |=0x40;	
	if(MchInf.KeyState&0x0100)
		tempkey |=0x80;

	if(MchInf.KeyState&0x80)
		tempkey |=0x100;
	
	if(MchInf.KeyState&0x01)
		tempkey |=0x200;
	if(MchInf.KeyState&0x02)
		tempkey |=0x400;	
	if(MchInf.KeyState&0x40)
		tempkey |=0x800;
	
	if(MchInf.KeyState&0x20)
		tempkey |=0x1000;
	if(MchInf.KeyState&0x10)
		tempkey |=0x2000;
	if(MchInf.KeyState&0x08)
		tempkey |=0x4000;	
	if(MchInf.KeyState&0x04)
		tempkey |=0x8000;	
//	if((MchInf.KeyState &0x8080)== 0x8080)//去掉耦合按键，后续查找原因
//		MchInf.KeyState &= 0x7fff;
	(*KeyInfDeal) = tempkey;
    //缓存按键状态
