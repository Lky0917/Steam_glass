...
#define  LIGHTALL     100
unsigned char DarkCal[20] = {0};//0  �����  1 Ӳ��  2 ����
...
//		DispAct[i] = 0xff;
  ...
  case 2: 
	  if((DispAct[2]&0x01)&&(nCalTime < DarkCal[1]))//yingdu
		  SEG1_ON_IO;
	  if((DispAct[2]&0x02)&&(nCalTime < DarkCal[2]))
		  SEG2_ON_IO;
	  if((DispAct[2]&0x04)&&(nCalTime < DarkCal[3]))//chugou
	  {
	  		SEG3_ON_IO;
	  //if(DispAct[2]&0x08)
		  SEG4_ON_IO;
  ...
  case 4: 
	  ...
	  if((DispAct[4]&0x08)&&(nCalTime < DarkCal[14]))//water
		  SEG4_ON_IO;
	  if((DispAct[4]&0x10)&&(nCalTime < DarkCal[14]))//LV1
		  SEG5_ON_IO;
	  if((DispAct[4]&0x20)&&(nCalTime < DarkCal[15]))//LV2
		  SEG6_ON_IO;
	  if((DispAct[4]&0x40)&&(nCalTime < DarkCal[16]))//tmr
		  SEG7_ON_IO;
	  if((DispAct[4]&0x80)&&(nCalTime < DarkCal[17]))//LV3
		  SEG8_ON_IO;
	  GRID5_ON_IO;
	  break;
  case 5: 
	  if((DispAct[5]&0x01)&&(nCalTime < DarkCal[18]))//B2
	  {	  SEG1_ON_IO;
	  //if(DispAct[5]&0x02)
		  SEG2_ON_IO;
	  }
	  if((DispAct[5]&0x04)&&(nCalTime < DarkCal[19]))//B1
	  {
	  	SEG3_ON_IO;
	  //if(DispAct[5]&0x08)
		  SEG4_ON_IO;
